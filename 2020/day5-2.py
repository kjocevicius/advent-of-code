...
def findEmptySeats(seatIds):
    sortedSeats = sorted(seatIds)
    previousSeatId = None
    emptySeats = []

    for seatId in sortedSeats:
        if (previousSeatId is None):
            previousSeatId = seatId
            continue

        diff = seatId - previousSeatId

        if (diff > 1):
            print("Found a space!", previousSeatId, seatId)
            for missingId in range(previousSeatId + 1, seatId):
                emptySeats.append(missingId)
...
emptySeats = findEmptySeats(seatIds)
